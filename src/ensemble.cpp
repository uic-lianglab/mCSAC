...
//	}
	...
	config.readInto( NumberofChains, "number_of_chains", 100);
	...
	for (int i=0; i<NumberofChains; i++)
	{
        ...
        int buffer;
	    string first = "chain_length_";
	    std::stringstream ss;
        ss << i;
        std::string second = ss.str();
        string result = first+second;
        cout<<result<<endl;
        config.readInto( buffer,result,100);
        ChainLengths.push_back(buffer);
	...
	cout<<cOutPath<<endl;
	cout<<fPersistenLength<<endl;
	cout<<fPackingDensity<<endl;
...
int i = 0;
	while (i<iSampleSize) {
	    cout<<"is it going into this loop"<<endl;
	 //   cout<<"is it going into this loop"<<endl;
		sprintf(buff, "%s_%04d", cPrefix, i);
		cout<< buff << endl;
		cout<<"hello"<<endl;
		ensemble.Resampling(1,1);
		cout<<"hello"<<endl;

        sprintf(cFN,"%s.pts",buff);
        ensemble.WritemChain(cFN,iNumNodes,1);
        cout<<"this happened write m chain"<<endl;
...
//      ensemble.WriteWeight(cFN);
      i++;
	        ...
	        cout<<"THIS HAPPENED ensemble resampling"<<endl;
